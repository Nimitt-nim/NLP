...
# Following lines of code will create dictionaries:: Key: sentenceID, Value: set of clean tokens in the sentence
count = 0 
processed_corpus= {}
for fileid in reuters.fileids():
...
''' Following lines of code will create two dictionaries:
Dict-1: Key: word, Value: set of context words with whom this word is present
Dict-2: Key: word, Value: no. of documents that word is present.
'''
...
for key in processed_corpus:
	for word in processed_corpus[key]:
		...
		word_to_doc_count[word]  = word_to_doc_count[word] + 1
...
# Following lines of code will update the previous dictionary by multiplying IDF values for the respective context word. 
for word in vectorized_word_to_word:
	for dim in range(0,len(vectorized_word_to_word[word])):
		vectorized_word_to_word[word][dim] = vectorized_word_to_word[word][dim]*math.log10(len(processed_corpus)/(word_to_doc_count[int_to_word[dim]]+1))
...
print(cosine_sim("student","school"))
print(cosine_sim("graduate","university"))
...
dataframe =pd.DataFrame(columns = word_list)
dataframe.loc[0] = vectorized_word_to_word["car"]
